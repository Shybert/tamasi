...
class SettingsGetters extends Getters<SettingsState> {
  getValidSettingValue<TSettingId extends keyof ISettings>(
    settingId: TSettingId
  ): ISettings[TSettingId]['defaultValue'] {
    const userSettingValue = this.state.userSettings[settingId]
    if (
      !isUndefined(userSettingValue) &&
      isValidSettingValue(settings[settingId], userSettingValue)
    )
