export default function formatBossTime (timeInMilliseconds: number, includeMilliseconds: boolean) {
  const date: Date = new Date(timeInMilliseconds)

  // Convert date to relevant time, then pad it with leading zeroes
  const hours: string = Math.floor(timeInMilliseconds / 3600000).toString().padStart(2, '0')
  ...
  const formattedTime: string = `${hours}:${minutes}:${seconds}${includeMilliseconds ? `.${milliseconds}` : ''}`
