function isModifier(key: string): boolean {
  ...
  return modifiersRegex.test(key)
}
...
export function validateKeybind(str: string): string | null {
  ...
  for (let i = 0; i < parts.length; i += 1) {
    ...
    if (isModifier(key)) {
      if (modifiers.includes(key))
        return 'Keybinds cannot have multiples of the same modifier'
      modifiers.push(key)
  ...
  if (modifiers.length > 0 && !hasKey)
    return 'A keybind cannot consist solely of modifiers'
  return null
}
